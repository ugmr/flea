		...
		<view class="u-tabbar__content safe-area-inset-bottom" :style="{
			...
			<view class="u-tabbar__content__item" v-for="(item, index) in list" :key="index" :class="{
				...
				<view :class="[
					...
					<u-icon
						:size="item.midButton ? midButtonSize : iconSize"
						:name="elIconPath(index)"
						img-mode="scaleToFill"
						:color="elColor(index)"
						:custom-prefix="item.customIcon ? item.customIcon : 'uicon'"
					></u-icon>
		...
		methods: {
			async clickHandler(index) {
				if(this.beforeSwitch && typeof(this.beforeSwitch) === 'function') {
					...
					let beforeSwitch = this.beforeSwitch.bind(this.$parent)(index);
					// 判断是否返回了promise
